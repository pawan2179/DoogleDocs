...
import ToastManager from "../components/organisms/toast-mananger";
...
interface ToastContextInterface{
  ...
  addToast: (
    { id, color, title, body, actions}:
    { id?: string, color?: ToastInterface['color'], title?: string, body?: string, actions?: Array<ActionInterface>},
    duration?: number
  ) => void;
...
export const ToastProvider = ({children}: ToastProviderInterface) => {
  ...
  const addToast = (
    { id = uuid(), color = 'primary', title, body, actions}: { id?: string, color?: ToastInterface['color'], title?: string, body?: string, actions?: Array<ActionInterface>},
    duration= TOAST_TIMEOUT
  ) => {
    setToasts((toasts) => [...toasts, {id, color, title, body, actions}]);
    setTimeout(() => {
      removeToast(id)
    }, duration);
  }
