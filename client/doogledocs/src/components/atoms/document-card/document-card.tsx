...
const DocumentCard = ({document, setDocument}: DocumentCardProps) => {
  const {userId} = useAuth();
  const navigate = useNavigate();

  const handleDocumentBtnClick = (event: React.MouseEvent<HTMLDivElement>, documentId: number) => {
  ...
  return (
    ...
    >
      <div className="h-80 w-full border-1 flex flex-col justify-between border-gray-300 hover:border-blue-500 rounded">
        <div className="w-fuoll h-full p-4 flex flex-col space-y-2">
          {skeleton}
        </div>
        <div className="w-full h-24 border-t pt-3 px-3">
